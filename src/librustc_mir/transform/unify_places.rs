...
impl<I: Idx> UnionFindSymRel<I> {
    fn union(&mut self, a: I, b: I) -> I {
        ...
        for i in child_relation.iter() {
            // HACK(eddyb) this is really bad, but used to propagate the relation.
            let i = self.finder.find(i);
            self.relation[root].insert(i);
            self.relation[i].insert(root);
            // FIXME(eddyb) check that this is even correct still...
        }
        ...
        let mut replacement_finder = {
            ...
            for (block, data) in mir.basic_blocks().iter_enumerated() {
                let mut add_conflicts_at = |past: &IdxSet<_>, future: &IdxSet<_>| {
                    ...
                    for i in past.iter() {
                        // If `i` was accessed, so should anything inside it.
                        // FIXME(eddyb) Mark those fields as dependent directly.
                        // FIXME(eddyb) Maybe relax this? Right now it'll introduce
                        // artificial conflicts even if `a` and `a.x` are used on
                        // separate codepaths. Not sure how to make it sound though.
                        for descendant in local_paths.descendants(i) {
                            // FIXME(eddyb) Here and elsewhere this might not need
                            // to be symmetric because `UnionFindSymRel` is itself.
                            conflicts[descendant].insert(i);
                            conflicts[i].insert(descendant);
                        }
...
impl<'a, 'tcx> DependencyCollector<'a, 'tcx> {
    ...
    fn collect_path(&mut self, path: PathId, prefixes: &mut Vec<PathId>) {
        ...
        // This path is dependent (if at all) on the longest prefix it
        // conflicts with, including conflicts included from child paths
        // that depend on this one, gathered by the recursion above.
        for (i, &parent) in prefixes.iter().rev().enumerate() {
            if self.conflicts.relates(path, parent) {
